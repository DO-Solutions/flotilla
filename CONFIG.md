...
| `treaties` | `True` | text | Official treaties: formal, PUBLIC pacts between two fleets — proposed and signed as actions, listed in every admiral's state.treaties, announced to all when formed, dissolved, or BROKEN (damaging a partner voids the pact instantly and names the breaker). The game records and announces treaties but never enforces their terms — negotiation happens over parley; this is the signature. Needs parley on. |
