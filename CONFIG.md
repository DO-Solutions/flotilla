...
| `rounds` | `1` | 1–10 | random_pairs / swiss: how many rounds to play. random_pairs reshuffles each round; swiss pairs on the standings so far. A useful swiss default is ceil(log2(participants)) — 3 rounds separates 8 admirals, 4 separates 16. |
...
| `historic_moments` | `False` | text | At tournament end, the narrator writes each participant's arc across the WHOLE bracket — synthesized from the per-series stories (series.historic_moments controls those and is REQUIRED for this — the finale synthesizes from the series stories, so validation refuses this switch alone). Beats cite {matchup, game, tick} anchors drawn from the already-validated series beats. One narrator call per participant, into tournament.json under historic_moments. Uses the series section's narrator model/timeout/chars knobs. |
