...
| `fx` | `sink` `flagSunk` `region` `parley` `signal` `treaty` `treatyEnd` `flash` `edgeArrow` | on-map VFX, one nested object per effect — see `docs/SPECTATOR.md` for the per-effect keys and defaults |
