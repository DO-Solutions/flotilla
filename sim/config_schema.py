...
SCHEMA = {
    ...
    "pacing": {
        ...
        "clock_jitter": dict(d=0, t="int", lo=0, hi=18000, doc="Anti-turtle: extend the match by a seeded-random 0..N extra ticks that admirals cannot predict, so a leader can never compute the exact bell and freeze through the final stretch. 0 = off (the classic fixed clock). Ignored in domination (no clock there). Reproducible: the same seed always draws the same extension."),
        "score_visibility": dict(d="exact", t="enum", opts=["exact", "banded", "hidden"], labels={"exact": "Exact", "banded": "Banded (nearest 100)", "hidden": "Own score only"}, doc="What admirals see of RIVAL scores. exact = live totals (classic). banded = rivals' scores rounded down to the nearest 100 — a leader can't compute that no rival can catch them, which keeps the endgame honest. hidden = each admiral sees only its own score. Spectators and the replay always see exact scores."),
