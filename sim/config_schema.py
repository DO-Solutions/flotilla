...
SCHEMA = {
    ...
    "economy": {
        ...
        "repair_period": dict(d=5,  t="int", lo=1,  hi=50,  doc="LEGACY trickle repair (+2 hull per this many ticks while docked, free) — active ONLY when shipyard_slots=0. With shipyards on, repairs are yard works: see repair_cost_pct / repair_ticks_pct."),
        ...
        "shipyard_cost": dict(d=45, t="int", lo=1, hi=500, doc="Treasury cost to build one ADDITIONAL yard slot ({\"build_yard\": true}) — priced like three ships by default, a real investment that pays off in build/repair throughput."),
        "shipyard_ticks": dict(d=300, t="int", lo=10, hi=5000, doc="Ticks to build an additional yard slot. Expansion is a harbor construction project, NOT yard work — it does not occupy a slot while building; one expansion at a time."),
        "shipyard_max": dict(d=4, t="int", lo=1, hi=8, doc="Ceiling on a fleet's total yard slots (starting slots included)."),
        "repair_cost_pct": dict(d=40, t="int", lo=0, hi=200, doc="Treasury cost of a yard repair, as % of the ship's class build cost for a FULL-hull restoration; the actual charge scales with missing hull (minimum 1). Default 40: even a nearly-dead hull costs well under half a new ship — sailing damaged ships home pays."),
        "repair_ticks_pct": dict(d=40, t="int", lo=0, hi=400, doc="Drydock time of a yard repair, as % of build_ticks for a FULL-hull restoration; scales with missing hull (minimum 1 tick). Default 40: even a nearly-dead hull is back at sea in under half a build time."),
        ...
        "refit_ticks": dict(d=-1,   t="int", lo=-1, hi=2000, doc="Drydock time per refit: the ship is held in port this many ticks before the conversion completes (paid up front). -1 (default) = MATCH build_ticks, so converting is not an instant fleet-swap — it costs less than a new ship (refit_cost < ship_cost) but takes the same time to build. 0 = immediate."),
    },
    "combat": {
    ...
    "pacing": {
        ...
        "window_wait_s": dict(d=45, t="int", lo=1, hi=600, doc="Catch-up pipelining only: wall-clock seconds each window stays open for replies. A live admiral answering within it decides every window; one that runs over misses windows until its reply arrives. Also the pacing unit for hold_full_window."),
        "hold_full_window": dict(d=True, t="bool", doc="Catch-up pipelining only: while any admiral is behind, each window lasts the FULL window_wait_s (wall-clock breathing room for stragglers to catch up without the sim racing ahead). Off = the window advances the moment every live admiral has replied — fastest sim, but laggards fall behind quicker."),
    },
    "scenario": {
        ...
        "design_points_max": dict(d=24, t="int", lo=8, hi=40, doc="flex_design only: the largest class anyone may design."),
        ...
        "program_chars": dict(d=3000, t="int", lo=200, hi=12000, doc="Max characters per squadron program; oversized programs are rejected at delivery. Roomy by default so admirals can write expressive, multi-phase programs."),
        "conn_examples": dict(d=5, t="int", lo=2, hi=5, doc="Worked examples in the conn API reference — the difficulty ladder. 2 = the bare survival pair (forager + lane split); 3 adds the PHASE MACHINE walkthrough; 4 adds FLAGSHIP HUNT; 5 (default) adds PACK HUNTER. Rank presets trim it: a Captain sails with the full tutorial, an Admiral with the bare tables."),
        ...
        "flag_speed":  dict(d=2,    t="int", lo=1, hi=6, doc="Flagship relocation speed, on the same scale as a ship's speed stat (a trawler is 3)."),
        ...
        "win":         dict(d="timed_score", t="enum", opts=["timed_score", "territory", "domination"],
                            doc="Victory condition. timed_score: cargo hauled + kills. territory: control points from holding named regions. domination: no clock — last admiral standing wins."),
        "domination_cap": dict(d=18000, t="int", lo=3000, hi=360000,
        ...
        "regions":     dict(d=16,   t="int", lo=4,  hi=48,  doc="Territory mode: number of named control regions (used only when region_size is 0)."),
        "region_size": dict(d=0,    t="int", lo=0,  hi=800, doc="Territory mode: AVERAGE region size in cells — the region count becomes map area / this (clamped 4-48). 0 = use the explicit `regions` count. ~324 on the default 96x54 map ≈ the classic 16 regions (count = 5184 / region_size)."),
        "region_symmetry": dict(d=True, t="bool", doc="Territory mode: place region seeds in 180°-mirrored pairs (like the contested-center wrecks) so no admiral's side of the map carves into systematically bigger regions. Off = fully random seeds."),
        "region_capture_ticks": dict(d=50, t="int", lo=0, hi=6000, doc="Ticks of SOLE possession needed to capture a region (10 ticks = 1s; default 50 = 5s). The clock runs only while NO enemy ship (the holder included) stands in the region — ship counts never matter. An enemy entering PAUSES progress where it is; progress resets only when every capturing ship leaves (the region stays with its owner). The viewer draws a filling pie; admirals see contested_by + capture_pct in state.regions. 0 = instant flips (pre-timer behavior)."),
    ...
    "admirals": {
        ...
        "memo_chars":  dict(d=2500, t="int", lo=200, hi=8000, doc="Hard character cap for post-game strategy memos, opening plans, AND per-admiral custom prompts. Models are told this budget explicitly; text is cut at the limit."),
    ...
    "tournament": {
        ...
        "players_per_match": dict(d=2, t="enum", opts=[2, 4, 8], doc="Fleets per matchup (single_elim is always 2)."),
