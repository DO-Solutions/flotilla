...
SCHEMA = {
    ...
    "scenario": {
        ...
        "treaties":    dict(d=True, t="bool", show_if={"parley": True}, doc="Official treaties: formal, PUBLIC pacts negotiated and signed INSIDE parley (a treaty field on a parley message). Two types — non_aggression (broken only when one side SINKS the other's ship; stray adjacency fire is not betrayal) and border (an agreed x/y line neither fleet crosses; broken when a signer SEES the other's ship beyond it, fog applies). Every pact is public in state.treaties; forming, dissolving, and breaking are announced to all admirals with the breaker named. The game records pacts, it never enforces conduct in advance. Needs parley on; border treaties also need contact_ttl > 0."),
    ...
    "series": {
        ...
        "historic_moments": dict(d=False, t="bool", doc="At series end, a NARRATOR model writes each LLM admiral's story through the series — key decisions, turning points, and where the admiral's own words show it surprised, frustrated, triumphant or resigned. Grounded in the record (the admiral's journal, memos, parley, and the event log): every beat must cite a real {game, tick} anchor or it is dropped before it renders. One narrator call per LLM admiral; collected in series.json under historic_moments. Costs count against the pre-flight estimate and the per-run ceiling. SPOILERS by construction — rendered behind the spoiler shield."),
        "historic_moments_model": dict(d="", t="str", show_if={"historic_moments": True}, doc="Which model narrates, independent of who played. Empty = the server's default narrator (Server tab); a config submitted through the dashboard gets that default injected. A bare runner config with historic_moments on must set this explicitly — validation refuses the silent alternative."),
        "historic_moments_timeout_s": dict(d=300, t="int", lo=30, hi=900, show_if={"historic_moments": True}, doc="Seconds per narrator call. Mirrors debrief_timeout_s — care beats speed here."),
        "historic_moments_chars": dict(d=2500, t="int", lo=500, hi=8000, show_if={"historic_moments": True}, doc="Cap on each admiral's rendered story text (the beat list is capped separately). Mirrors memo_chars."),
    },
    "tournament": {
        "format":      dict(d="round_robin", t="enum",
                            opts=["round_robin", "random_pairs", "single_elim", "swiss"],
                            doc="Bracket structure. round_robin: every combination plays. random_pairs: seeded random pairings per round. single_elim: knockout bracket (2-player matchups, participants must be a power of 2). swiss: fixed number of rounds, nobody eliminated — each round pairs admirals on similar records (leaders meet leaders), rematches avoided where possible, and with an odd field the bye goes to whoever has had fewest. Best when you want a ranking from every participant without the N-squared cost of a full round robin."),
        "players_per_match": dict(d=2, t="enum", opts=[2, 4, 8], show_if={"format": {"not": ["single_elim", "swiss"]}}, doc="Fleets per matchup (single_elim and swiss are always 2)."),
        ...
        "rounds":      dict(d=1,    t="int", lo=1,  hi=10,  show_if={"format": ["random_pairs", "swiss"]}, doc="random_pairs / swiss: how many rounds to play. random_pairs reshuffles each round; swiss pairs on the standings so far. A useful swiss default is ceil(log2(participants)) — 3 rounds separates 8 admirals, 4 separates 16."),
        ...
        "historic_moments": dict(d=False, t="bool", doc="At tournament end, the narrator writes each participant's arc across the WHOLE bracket — synthesized from the per-series stories (series.historic_moments controls those and is REQUIRED for this — the finale synthesizes from the series stories, so validation refuses this switch alone). Beats cite {matchup, game, tick} anchors drawn from the already-validated series beats. One narrator call per participant, into tournament.json under historic_moments. Uses the series section's narrator model/timeout/chars knobs."),
    },
}
